...
function renderSidebar(filter = "") {
  ...
  categories
    ...
    .forEach((c) => {
      ...
      el.className =
        "category-item " + (selectedCategory === c.name ? "active" : "");
...
function showAll() {
  ...
  window.scrollTo({
    top: document.querySelector(".categories-wrap").offsetTop - 10,
    behavior: "smooth",
  });
}
...
function renderItems(category) {
  itemsGrid.innerHTML = category.items
    .map(
      (item, index) => `
...
function updateTimer(category, index) {
  ...
  const btn = document.querySelector(
    `.bid-btn[data-category="${CSS.escape(category.name)}"][data-index="${index}"]`,
  );
  if (!el || !state) return;
